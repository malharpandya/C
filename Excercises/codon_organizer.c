...
void sortCodons(Codon codons[], int num_codons){
    int count[64] = {0}, index = 0;
    for (int i = 0; i < num_codons; i++) count[codons[i][0]*16 + codons[i][1]*4 + codons[i][2]]++;
    for (int i = 0; i < 64; i++){
        while (count[i] != 0){
            for (int exp = 2; exp >= 0; exp--) codons[index][exp] = (i / (int) pow(4, 2 - exp))%4;
            count[i]--; index++;}}}
